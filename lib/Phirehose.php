...
use Exception;
use PhirehoseException;
use PhirehoseNetworkException;
use PhirehoseConnectLimitExceeded;
...
abstract class Phirehose
{
    ...
    /**
     * @internal Moved from being a const to a variable, because some methods
     * (user and site) need to change it.
     */
    protected $URL_BASE    = 'https://stream.twitter.com/1.1/statuses/';

    /**
     * Any twitter username. When using oAuth, this is the 'oauth_token'
     * @var string
     */
    public $username;

    /**
     * Any twitter password. When using oAuth this is you oAuth secret
     * @var string
     */
    public $password;
     ...
     * METHOD_LINKS, METHOD_USER, METHOD_SITE. Note: the method might cause
     * the use of a different endpoint URL.
     * Formats are: FORMAT_JSON, FORMAT_XML
     * @see Phirehose::METHOD_SAMPLE
     * @see Phirehose::FORMAT_JSON
     */
    public $method = Phirehose::METHOD_SAMPLE;
    public $format = Phirehose::FORMAT_JSON;
    public $lang = false;
    ...
    /**
     * Total number of seconds (fractional) spent in the enqueueStatus() calls
     * (i.e. the customized function that handles each received tweet).
     *
     * @var float
     */
    ...
    /**
     * Time spent on each call to enqueueStatus() (i.e. average time spent,
     * in milliseconds, spent processing received tweet).
     *
     ...
     * Mentions ("Hello @user!") and implicit replies
     * ("@user Hello!" created without pressing the reply button) are not matched.
     * It is up to you to find the integer
    ...
    /**
     * Specifies keywords to track. Track keywords are case-insensitive logical ORs.
     * Terms are exact-matched, ignoring
     * punctuation. Phrases, keywords with spaces, are not supported.
     * Queries are subject to Track Limitations.
     * Applies to: METHOD_FILTER
     *
    ...
    /**
     * Specifies a set of bounding boxes to track as an array of 4 element
     * lon/lat pairs denoting <south-west point>, <north-east point>.
     * Only tweets that are both created using the Geotagging API and are placed
     * from within a tracked bounding box will be included in the stream.
     * The user's location field is not used to filter tweets. Bounding boxes
     * are logical ORs and must be less than or equal to 1 degree per side.
     * A locations parameter may be combined with
     * track parameters, but note that all terms are logically ORd.
     *
     * NOTE: The argument order is Longitude/Latitude (to match the Twitter API
     * and GeoJSON specifications).
     *
     * Applies to: METHOD_FILTER
     *
    ...
    /**
     * Returns an array of 4 element arrays that denote the monitored location
     * bounding boxes for tweets using the
     * Geotagging API.
     *
     * @see setLocations()
     * @return array
    ...
    /**
     * Convenience method that sets location bounding boxes by an array
     * of lon/lat/radius sets, rather than manually specified bounding boxes.
     * Each array element should contain 3 element subarray containing a latitude, longitude and
    ...
    /**
     * Sets the number of previous statuses to stream before transitioning to the live stream.
     * Applies only to firehose and filter + track methods. This is generally
     * used internally and should not be needed by client applications.
    ...
    /**
     * Connects to the stream API and consumes the stream.
     * Each status update in the stream will cause a call to the
     * handleStatus() method.
     *
     * Note: in normal use this function does not return.
     * If you pass $reconnect as false, it will still not return in normal use:
     * it will only return if the remote side (Twitter) close the socket.
     * (Or the socket dies for some other external reason.)
     *
     * @see handleStatus()
    ...
    {
      $this->log('Consume rate: ' . $this->statusRate . ' status/sec ('
        . $this->statusCount . ' total), avg ' . 'enqueueStatus(): '
        . $this->enqueueTimeMS . 'ms, avg checkFilterPredicates(): '
        . $this->filterCheckTimeMS . 'ms (' . $this->filterCheckCount
        . ' total) over ' . $this->avgElapsed . ' seconds, max stream idle period: '
    ...
    /**
     * Returns the last error message (TCP or HTTP) that occured with the streaming API or client.
    ...
    /**
     * Returns the last error number that occured with the streaming API or client.
     * Numbers correspond to either the fsockopen() error states (in the case of TCP errors)
     * or HTTP error codes from Twitter (in the case of HTTP errors).
     *
     * State is cleared upon successful reconnect.
     *
     * @return string
    ...
    protected function connect()
    {
        ...
        do {
            ...
            if (($this->method == self::METHOD_FILTER || $this->method == self::METHOD_USER)
                && count($this->trackWords) > 0) {
             ...
             * It'd be nice to use stream_context_create with the native
            ...
            if (!$this->conn || !is_resource($this->conn)) {
                ...
                $this->log('TCP failure ' . $connectFailures . ' of '
            ...
            if ($httpCode != 200) {
                ...
                $this->log('HTTP failure ' . $connectFailures . ' of ' . $this->connectFailuresMax
                    . ' connecting to stream: ' . $errStr . '. Sleeping for ' . $httpRetry . ' seconds.','info');
                sleep($httpRetry);
                continue;
    ...
    /**
     * Method called as frequently as practical (every 5+ seconds) that is
     * responsible for checking if filter predicates (ie: track words or follow IDs) have changed.
     * If they have, they should be set using the setTrack() and setFollow()
     * methods respectively within the overridden implementation.
     *
     * Note that even if predicates are changed every 5 seconds, an actual reconnect
     * will not happen more frequently than
     * every 2 minutes (as per Twitter Streaming API documentation).
     *
     * Note also that this method is called upon every connect attempt,
     * so if your predicates are causing connection
    ...
    /**
     * Basic log function that outputs logging to the standard error_log() handler.
     * This should generally be overridden to suit the application environment.
     *
     * @see error_log()
    ...
    /**
     * Reconnects as quickly as possible. Should be called whenever a reconnect is
     * required rather that connect/disconnect to preserve streams reconnect state
     */
    ...
    /**
     * This is the one and only method that must be implemented additionally.
     * As per the streaming API documentation, statuses should NOT be processed
     * within the same process that is performing collection
     *
     * @param string $status
     */
